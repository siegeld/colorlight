...
Usage:
    ./bench_stream.py --host 10.11.6.72 sweep
    ./bench_stream.py --host 10.11.6.72 isolate
    ./bench_stream.py --host 10.11.6.72 drop
"""
import argparse
import json
import socket
    ...
    for f in range(nframes):
        for i in range(chunks):
            if omit is not None and i == (omit if omit >= 0 else chunks + omit):
                continue
            s.sendto(
                struct.pack(HEADER_FMT, b"BM", f & 0xFFFF, i, chunks, w, h) + chunk,
                (host, port),
            )
            sent += 1
            if delay > 0:
...
def main():
    ...
    p.add_argument("--host", default="10.11.6.72")
